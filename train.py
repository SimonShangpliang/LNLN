...
def main():
    ...
    # Use -float('inf') or 0.0 for "higher is better" metrics
    # Use float('inf') for "lower is better" metrics (like MAE)
    initial_worst_results = {
        'Has0_acc_2': 0.0,
        'Has0_F1_score': 0.0,
        'Non0_acc_2': 0.0,
        'Non0_F1_score': 0.0,
        'MAE': float('inf'),
        'Corr': -1.0, # Correlation can be negative, so -1.0 is a good "worst"
        'Mult_acc_2': 0.0,
        'F1_score': 0.0,
        'Mult_acc_3': 0.0,
        'Mult_acc_5': 0.0,
    ...
    # Use .copy() so they are two independent dictionaries
    best_valid_results = initial_worst_results.copy()
    best_test_results = initial_worst_results.copy()
    ...
    for epoch in range(1, args['base']['n_epochs']+1):
        ...
        if args['base']['do_validation']:
            ...
            best_valid_results = get_best_results(valid_results, best_valid_results, epoch, model, optimizer, ckpt_root, seed, save_best_model=True)
